      ...
      <article className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 py-8 px-6 border-b border-border/40 z-10">
        <div className="flex-1 min-w-0">
          <header className="flex items-start gap-3">
            <span className="text-4xl font-bold text-muted-foreground/20 leading-none flex items-center min-h-[3.5rem] select-none" aria-hidden="true">
            ...
            <div className="flex-1">
              ...
              <footer className="flex flex-wrap items-center gap-3 mt-3 text-sm text-muted-foreground">
                <div className="flex items-center gap-1.5">
                  <Calendar className="w-3.5 h-3.5 opacity-60 transition-transform duration-300 group-hover:-rotate-12 group-hover:text-accent" aria-hidden="true" />
                  <time dateTime={post.date}>
                    <AnimatedNumber value={post.publishedAt} duration={dateDuration} />
                  </time>
                </div>
                
                <span className="text-muted-foreground/30" aria-hidden="true">·</span>
                
                <div className="flex items-center gap-1.5" aria-label="Estimated read time">
                  <Clock className="w-3.5 h-3.5 opacity-60 transition-transform duration-300 group-hover:rotate-45 group-hover:text-accent" aria-hidden="true" />
                  <span className="tabular-nums font-medium">
                    <AnimatedNumber value={post.readTime} duration={readTimeDuration} />
                  </span>
                </div>
                ...
                {post.views && (
                  <>
                    <span className="text-muted-foreground/30" aria-hidden="true">·</span>
                    <div className="flex items-center gap-1.5" aria-label={`${post.views} views`}>
                      <Eye className="w-3.5 h-3.5 opacity-60 transition-transform duration-300 group-hover:scale-110 group-hover:text-accent" aria-hidden="true" />
                      <span className="tabular-nums font-medium">
                        <AnimatedNumber value={post.views.toLocaleString()} duration={viewsDuration} />
                      </span>
                    </div>
                  </>
                )}
