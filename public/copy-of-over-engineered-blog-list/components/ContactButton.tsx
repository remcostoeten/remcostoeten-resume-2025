...
const ContactPopoverContext = React.createContext<ContactPopoverContextType | undefined>(undefined)

function useContactPopover() {
	const context = React.useContext(ContactPopoverContext)
	if (!context) {
		throw new Error('useContactPopover must be used within a ContactPopoverProvider')
	}
	return context
}
...
const ContactPopoverRoot = React.forwardRef<HTMLDivElement, ContactPopoverRootProps>(
...
const ContactPopoverTrigger = React.forwardRef<HTMLButtonElement, ContactPopoverTriggerProps>(
	({ children, className }, ref) => {
		...
		return (
			<button ref={combinedRef} className={className} onClick={openPopover} disabled={isOpen}>
				{children}
			</button>
		)
	}
...
const ContactPopoverContent = React.forwardRef<HTMLDivElement, ContactPopoverContentProps>(
	({ className }, ref) => {
		const {
			isOpen,
			closePopover,
