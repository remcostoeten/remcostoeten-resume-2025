...
const ShareButtons: React.FC<{ title: string; slug: string }> = ({ title, slug }) => {
	...
	return (
		<div
			className='flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-8 border-y border-border/40 my-12 animate-enter'
	...
	useEffect(() => {
		if (contentRef.current && htmlContent && typeof Prism !== 'undefined') {
			...
			preTags.forEach((pre) => {
				...
				wrapper.className =
					'relative my-8 rounded-xl overflow-hidden bg-[#1e1e1e] border border-border/30 shadow-2xl group/code'

				// Header
				const header = document.createElement('div')
				header.className =
					'flex items-center justify-between px-4 py-3 bg-[#252526] border-b border-border/10'
				...
				label.className =
					'absolute left-1/2 -translate-x-1/2 text-[11px] font-mono font-medium text-muted-foreground/60 uppercase tracking-widest pointer-events-none select-none'
				label.textContent = language

				// Copy Button Container
				const copyBtn = document.createElement('button')
				copyBtn.className =
					'opacity-0 group-hover/code:opacity-100 transition-opacity p-1.5 rounded-md hover:bg-white/10 text-muted-foreground hover:text-white'
	...
	return (
        ...
        '
			...
			{relatedPosts.length > 0 && (
				<div className='mt-8 pt-12 animate-enter' style={{ animationDelay: '700ms' }}>
